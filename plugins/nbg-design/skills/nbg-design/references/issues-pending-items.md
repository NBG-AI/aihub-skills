...
- Affected files: `scripts/lib/deck-menu.js` (v6, ~120 KB per deck), `SKILL.md`, `config/pi-agent-nbg-design.yaml`, `scripts/README.md`, plugin README, references, manifests (1.8.0 → 1.9.0); workspace `test_scripts/deck-menu-roundtrip.mjs` (+37 checks).
- Verification: round-trip test on the three real decks, 128 checks each, all passing (a `var sel` local in the keyboard handler shadowed the shape selection through hoisting and made the Tab branches throw silently — found by the test, renamed); saved copies pass `verify-deck.mjs --strict` and `export-pdf.mjs` (15 / 11 / 17 pages).
