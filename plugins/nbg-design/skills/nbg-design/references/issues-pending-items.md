...
- Verification: round-trip test on the three real decks, 135 checks each, all passing (a `var sel` local in the keyboard handler shadowed the shape selection through hoisting and made the Tab branches throw silently — found by the test, renamed); saved copies pass `verify-deck.mjs --strict` and `export-pdf.mjs` (15 / 11 / 17 pages).
