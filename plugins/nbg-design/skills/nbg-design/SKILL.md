...
- **One floating toolbar** — the text formatting row and the shape & arrange rows share a single panel with one grip, one ⚓ and one ⧉ in a side strip; each row appears with what applies (text being edited or selected text blocks; a selected shape) and can be hidden on its own (its ✕, the menu's *Toolbars* section). The panel floats next to the selection and can be dragged anywhere by its grip. A dragged toolbar is **anchored**: it opens at that spot from then on (remembered per deck in the browser) and shows ⚓ next to its grip; click ⚓ (or double-click the grip) to release it, and it follows the selection again. ⧉ detaches the whole toolbar into a window of its own.
