...
- **Structure / HTML panel** — *Show structure & HTML* in the menu, the `</>` button on the shape toolbar, or Ctrl/Cmd+Shift+O / H. A movable (by its grip, or by dragging its header's empty space or its "Slide N" title), resizable panel docked to the right with three tabs; ▾ in its header collapses it to the header row (▸ expands; remembered per deck), and rows wider than the panel scroll sideways with a visible scrollbar. **Right-click a row** in the Outline or Tree to ask the assistant about that element or to have it changed (see "Ask the assistant"). **Outline**: only the slide's shapes — cards, text blocks, images — nested by containment, each with a checkbox, a kind icon, a readable label (the text, the image's alt, or tag.class), its size and a group badge; tick boxes to select several, *All* / *None*, a text filter, click a name to select it alone, Shift+click adds, Ctrl/Cmd+click toggles, Space toggles the focused row; the selection is highlighted on the slide and in the list. A selection never holds a container together with something inside it (the inner one is left out, with a note). **Tree**: the slide's elements (tag, id, classes, attributes, text previews) synced with the visual selection both ways — selecting on the slide reveals and highlights the row; clicking a row selects exactly that element on the slide (any level, so it is also a way through nested shapes), Shift+click adds it, double-click edits its text, hovering outlines it, arrow keys and Enter navigate; edited elements carry a dot. **Code**: the selected element's exact source, **editable** — Apply (Ctrl/Cmd+Enter) keeps the element's type, strips `<script>`, inline event handlers and `javascript:` URLs, and records the result like every other edit (style, group, markup, and an `attrs` record for the other attributes), so it persists, lands in the saved copy and is removed by Discard. Copy copies the source. Right-clicking a row opens a small *Ask about …* popup for that element (request, Answer / Replace, prompt, attachments); it stays open until you close it (✕, Cancel, Esc), shows the status and the answer itself — Copy, Undo after a replacement, *Open in the assistant panel* — and never opens the assistant panel on its own. Since v1.15.0 the panel has two tabs: *Outline*, and *Tree* with the selected element’s editable source in its bottom half, under a draggable splitter whose position is remembered per deck (double-click it to reset).
