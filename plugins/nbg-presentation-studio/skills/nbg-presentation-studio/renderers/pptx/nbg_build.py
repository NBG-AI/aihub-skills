...
# Consolidated skill layout: renderers/pptx/ -> ../../theme/nbg/assets
# (override with the NBG_ASSETS_DIR env var to point at a different theme)
ASSETS_DIR = Path(
    os.environ.get(
        "NBG_ASSETS_DIR",
        SCRIPT_DIR.parent.parent / "theme" / "nbg" / "assets",
    )
)
...
def _add_bumper_pill(slide, text, x=0.37, y=0.30):
    """Add bumper as a filled rounded-rect pill with white text (NBG pattern).

    Pill: 1.3x0.19, fill #007B85, text 9pt Bold white ALL CAPS.
    Height is kept at 0.19" so the validator's content-spacing check treats it as
    a header chip (skips elements < 0.2" tall) rather than as mis-spaced content.
    """
...
def create_chart_slide(prs, content, page_number, chart_type="bar"):
    ...
    title_y = 0.55 if bumper else 0.5
...
def build_presentation(outline_path, output_path):
    ...
    # Slides may be nested under `presentation:` OR provided at the top level
    # alongside a `presentation:` metadata block (the form all examples use).
