...
// Consolidated skill layout: data-URI assets live alongside this script in ./assets.
// Override with --assets <dir> to point at a different theme's data-URI assets.
const DEFAULT_ASSETS = resolve(__dirname, 'assets');
...
Usage: node embed-assets.mjs <deck.html> [-o <out.html>] [--assets <dir>]
  ...
  --assets <dir>  Override the assets directory (default: the data-URI assets next to this script, ./assets).
...
function main() {
  ...
  if (!existsSync(assetsDir)) {
    fail(`assets directory not found: ${assetsDir}\n` +
         `  The renderers/html/assets folder (data-URI files) must travel with this script.\n` +
